...
StateStore.prototype.load = function(req, cb) {
  ...
  function loaded(err, state) {
    ...
    state.locals = {};
    if (state.webMessaging) {
      state.locals.webMessaging = state.webMessaging;
      delete state.webMessaging;
    }
...
StateStore.prototype.store = function(req, cb) {
  ...
  this._serializeClientFunc(txn.client, function(err, client) {
    ...
    state.req = txn.req;
    if (txn.locals.webMessaging) {
      state.webMessaging = txn.locals.webMessaging;
    }
